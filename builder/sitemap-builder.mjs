...
const buildUrl = (url, item) => {
  ...
  let path
  if (pathIndex > -1) {
    path = item.openingElement.attributes[pathIndex].value.value
  } else {
    path = ''
  }
...
const buildSitemap = (fileName, buildPath, url) => {
        ...
        if (item.consequent.type === 'JSXFragment') {
          ...
          if (
            ...
            (item.consequent.openingElement.name.name === 'Router' ||
              ...
              item.consequent.openingElement.name.name === 'Routes')
          ) {
        ...
        if (item.type === 'JSXFragment') {
          ...
          if (
            ...
            (item.openingElement.name.name === 'Router' ||
              ...
              item.openingElement.name.name === 'Routes')
          ) {
            //if it exists, filter it for only elements that are routes and return it.
  ...
  if (router !== undefined) {
    ...
    router.forEach((item) => {
      const newUrl = buildUrl(url, item)
      urls.add(newUrl)

      item.children
        ?.filter((x) => x.type === 'JSXElement')
        ?.forEach((item) => {
          const childUrl = buildUrl(newUrl, item)
          urls.add(childUrl)
        })
    })
